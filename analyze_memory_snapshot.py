...
def analyze_memory_snapshot(file_path):
    ...
    try:
        ...
        if isinstance(snapshot, dict) and 'segments' in snapshot:
            ...
            print(f"🔍 Memory segments: {len(segments)} segments")
            ...
            print("❌ No memory segments found in snapshot")
...
def analyze_directory(dir_path):
    ...
    if not pickle_files:
        print(f"❌ No pickle files found in: {dir_path}")
        return
    ...
    print(f"\n{'='*80}")
    print(f"📁 Most recent file: {os.path.basename(last_file)}")
    print(f"📁 Full path: {last_file}")
    print(f"{'='*80}")
